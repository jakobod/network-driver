...
struct dummy_multiplexer : public multiplexer {
  ...
  void add(socket_manager_ptr mgr, operation) override {
    add_called = true;
    last_manager = mgr.get();
  }

  void enable(socket_manager_ptr mgr, operation) override {
    enable_called = true;
    last_manager = mgr.get();
  }

  void disable(socket_manager_ptr mgr, operation, bool) override {
    disable_called = true;
    last_manager = mgr.get();
  }
...
struct pollset_updater_test : public ::testing::Test, public dummy_multiplexer {
  ...
  void write_to_pipe(pollset_updater::opcode code, socket_manager* ptr) {
...
TEST_F(pollset_updater_test, handle_add) {
  ...
  write_to_pipe(pollset_updater::add_code, mgr.get());
  updater.handle_read_event();
...
TEST_F(pollset_updater_test, handle_enable) {
  pollset_updater updater{pipe_reader, this};
  EXPECT_EQ(updater.init(), util::none);
  auto mgr = util::make_intrusive<dummy_manager>(invalid_socket, this);
  mgr->ref();
  EXPECT_EQ(mgr->ref_count(), 2);
  write_to_pipe(pollset_updater::enable_code, mgr.get());
  updater.handle_read_event();
  EXPECT_EQ(mgr->ref_count(), 1);
  EXPECT_EQ(last_error, util::none);
  EXPECT_TRUE(enable_called);
  EXPECT_EQ(last_manager, mgr.get());
}
