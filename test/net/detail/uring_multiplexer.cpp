...
/// Mock socket manager for uring_multiplexer tests.
class MockSocketManager : public detail::uring_manager {
public:
  MockSocketManager(net::socket handle, detail::multiplexer_base* mpx)
    : detail::uring_manager{handle, mpx} {
    // nop
  }

  MOCK_METHOD(manager_result, enable, (operation), (override));
  MOCK_METHOD(manager_result, handle_completion, (operation, int), (override));
  MOCK_METHOD(manager_result, handle_timeout, (uint64_t), (override));
};
...
// /// Real implementation of socket manager for integration-style tests.
...
//   bool poll_until(const std::function<bool()>& predicate, bool blocking =
//   false,
...
//   mpx.set_thread_id();
//   mpx.shutdown();
//   // No shutdown should have been initiated - All managers must still be
//   present ASSERT_EQ(mpx.num_socket_managers(), default_num_socket_managers);
...
TEST(UringMultiplexerMockTest, mpx_shuts_down_correctly_with_mock) {
  // Create multiplexer with mock socket managers
  detail::uring_multiplexer mpx;
  util::config cfg;

  auto factory = [](net::socket handle, detail::multiplexer_base* mpx) {
    auto mock = util::make_intrusive<MockSocketManager>(handle, mpx);
    EXPECT_CALL(*mock, enable).Times(1);
    EXPECT_CALL(*mock, handle_completion).Times(0);
    EXPECT_CALL(*mock, handle_timeout).Times(0);
    return mock;
  };
  EXPECT_EQ(mpx.init(std::move(factory), cfg), util::none);
  const auto default_num_socket_managers = mpx.num_socket_managers();
  mpx.set_thread_id();
  mpx.shutdown();
  ...
  mpx.set_thread_id(std::this_thread::get_id());
  ASSERT_TRUE(
    test::poll_until([&] { return mpx.num_socket_managers() == 0; }, mpx));
}
