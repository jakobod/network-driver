    ...
    switch (op) {
      case operation::read: {
        ...
        auto [success, submission_id]
          = manager_base::mpx<detail::uring_multiplexer>()->submit_read(
            *this, read_buffer_);
...
private:
  manager_result handle_read_completion(int res) {
  ...
  manager_result handle_write_completion(int res) {
...
TEST_F(uring_multiplexer_test, event_handling) {
  ...
  {
    const auto result = test::write_all(guard.get(), buf);
    ASSERT_EQ(result, manager_result::done);
  }
  ...
  {
    const auto result = test::read_all(guard.get(), buf);
    ASSERT_EQ(result, manager_result::ok);
  }
}
