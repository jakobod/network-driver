...
// unsigned char dest_mac[ETHER_ADDR_LEN] = {0xe4, 0x54, 0xe8, 0x81, 0xbc,
// 0x5f}; unsigned char src_mac[ETHER_ADDR_LEN] = {0x78, 0x7b, 0x8a, 0xaf, 0x67,
// 0x9a};
...
// // Set some options on the bpf device, then get the length of the kernel
// buffer int get_buf_len(int bpf) {
//   int buf_len = 1;
...
//     // Fill frame payload
//     std::cerr << "Copying payload from " << start << ", length" <<
//     bytes_to_send
//               << std::endl;
...
//               << " of maximum net frame length" << ETHER_MAX_LEN -
//               ETHER_CRC_LEN
//               << std::endl;
...
// // Create a simple ramp so we can check the splitting of data across frames
// on
