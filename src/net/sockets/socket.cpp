...
void close(socket hdl) noexcept {
...
void shutdown(socket sock, connection_channel how) noexcept {
  LOG_DEBUG("shutdown ", NET_ARG2("socket", sock.id), ", ",
            NET_ARG(static_cast<int>(how)));
  if (sock != invalid_socket) {
    ::shutdown(sock.id, static_cast<int>(how));
  }
}

util::error bind(socket hdl, ip::v4_endpoint ep) noexcept {
...
bool nonblocking(socket hdl, bool new_value) noexcept {
...
bool reuseaddr(socket sock, bool new_value) noexcept {
